...
# CLAUDE: shared base for the RPGM Node2D family (event, script, player) —
# defines the lazy cached getters once instead of each class reimplementing
# them. _cached resolves a ref on first use and re-resolves if it was freed.
# Mover and Lambdas extend Node (not Node2D) so they can't share this base
# without changing their node types in every scene.
var _refs := {}
...
func get_mover() -> _RPGM_Mover: return _cached(&"mover", func(): return find_child("_RPGM_Mover")) as _RPGM_Mover
